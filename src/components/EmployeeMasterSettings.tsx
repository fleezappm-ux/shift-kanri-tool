...
export function EmployeeMasterSettings({ employees, onSave }: { employees: EmployeeMasterItem[]; onSave: (items: EmployeeMasterItem[]) => Promise<void> }) {
  ...
  const add = () => setDrafts(items => [...items, { id: crypto.randomUUID(), name: "", displayName: "", displayOrder: items.length + 1, active: true, aliases: [] }]);
  ...
  return <section className="space-y-4 rounded-2xl border border-slate-200 bg-slate-50 p-5">
    ...
    <div className="space-y-2">{activeDrafts.map((item, index) => <div key={item.id} className="grid items-center gap-2 rounded-xl border bg-white p-3 sm:grid-cols-[42px_1fr_1fr_auto]">
