...
export function LeaveRequestManager({ requests, loading, onStatusChange }: Props) {
  ...
  const pending = visible.filter(item => item.status === "申請中" && item.type !== "希望なし" && item.type !== "訂正依頼").length;
  ...
  return <section className="leave-manager">
    ...
    {visible.length === 0 ? <p>この期間の希望申請はありません</p> : <div className="leave-manager-list">
      {visible.sort((a,b)=>a.date.localeCompare(b.date)).map(item => <div key={item.id} className={`leave-manager-row status-${item.status}`}>
        ...
        {item.status === "申請中" && item.type !== "希望なし" && item.type !== "訂正依頼" && <div className="leave-manager-actions"><button disabled={loading} onClick={()=>setTarget({request:item,status:"承認"})}><Check className="w-4 h-4"/>承認</button><button disabled={loading} onClick={()=>setTarget({request:item,status:"却下"})}><X className="w-4 h-4"/>却下</button></div>}
      </div>)}
    </div>}
