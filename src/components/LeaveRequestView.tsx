...
export function LeaveRequestView({ employees, dates, requests, remarks, locked, loading, operatorId, onSubmit, onCancel }: Props) {
  ...
  const submitNote = async () => {
    ...
    try {
      for (const [date, type] of draftEntries) {
        await onSubmit({ employeeId: operatorId, employeeName, date, type, comment, commentVisibility: "all" });
      }
      setDrafts({});
      setComment("");
    } finally {
  ...
  return (
    <div className="leave-request-page space-y-4 pb-5">
      ...
      <section className="leave-request-card">
        {locked && <p className="leave-locked-message">確定済みのシフト期間は休み希望を変更できません。</p>}
        {employeeName && <>
          ...
          {!locked && <div className="leave-note-card">
            <h2>提出ノート</h2>
            {draftEntries.length === 0 ? <p className="leave-empty">カレンダーの日付から希望種別を選ぶと、ここに追加されます。</p> : <div className="leave-note-lines">
              {draftEntries.map(([date, type]) => <div key={date}><strong>{format(new Date(`${date}T00:00:00`), "M/d（E）", { locale: ja })}</strong><span>{type}</span><button onClick={() => updateDraft(date, "")} aria-label={`${date}を削除`}><Trash2 className="w-4 h-4" /></button></div>)}
            </div>}
            <label className="leave-field-label">コメント（任意）</label>
            <textarea value={comment} onChange={event => setComment(event.target.value)} placeholder="まとめて伝えたいことがあれば入力してください" rows={3} />
            <Button className="w-full h-12 font-bold" disabled={draftEntries.length === 0 || loading || submitting} onClick={submitNote}><Send className="w-4 h-4 mr-2" />{submitting ? "提出中…" : "このノートを提出"}</Button>
          </div>}
        </>}
      </section>
