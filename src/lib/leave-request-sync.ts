...
export async function fetchLeaveRequests(periodStart: string, periodEnd: string): Promise<LeaveRequest[]> {
  const json = await request("getShiftLeaveRequests", { periodStart, periodEnd });
...
export async function submitLeaveRequest(input: {
  ...
  const json = await request("saveShiftLeaveRequest", { request: input });
  return json.request as LeaveRequest;
}

export async function cancelLeaveRequest(id: string): Promise<LeaveRequest> {
  const json = await request("cancelShiftLeaveRequest", { id });
  return json.request as LeaveRequest;
}

export async function updateLeaveRequestStatus(id: string, status: LeaveRequestStatus): Promise<LeaveRequest> {
  const json = await request("updateShiftLeaveRequestStatus", { id, status }, true);
  return json.request as LeaveRequest;
}
